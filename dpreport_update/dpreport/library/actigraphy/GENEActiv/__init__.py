...
GZ_FILE_REGEX = re.compile(r'(?P<subject>\w+)_(?P<hand>\w+)\s(?P<position>\w+)_(?P<serialnum>\w+)_(?P<year>[0-9]{4})-(?P<month>[0-9]{2})-(?P<day>[0-9]{2})\s(?P<hour>[0-9]{2})-(?P<minute>[0-9]{2})-(?P<second>[0-9]{2}).csv.gz')
FILE_REGEX = re.compile(r'(?P<subject>\w+)_(?P<hand>\w+)\s(?P<position>\w+)_(?P<serialnum>\w+)_(?P<year>[0-9]{4})-(?P<month>[0-9]{2})-(?P<day>[0-9]{2})\s(?P<hour>[0-9]{2})-(?P<minute>[0-9]{2})-(?P<second>[0-9]{2}).csv')
GZ_FILE_REGEX_2 = re.compile(r'(?P<subject>\w+)__(?P<serialnum>\w+)_(?P<year>[0-9]{4})-(?P<month>[0-9]{2})-(?P<day>[0-9]{2})\s(?P<hour>[0-9]{2})-(?P<minute>[0-9]{2})-(?P<second>[0-9]{2}).csv.gz')
FILE_REGEX_2 = re.compile(r'(?P<subject>\w+)__(?P<serialnum>\w+)_(?P<year>[0-9]{4})-(?P<month>[0-9]{2})-(?P<day>[0-9]{2})\s(?P<hour>[0-9]{2})-(?P<minute>[0-9]{2})-(?P<second>[0-9]{2}).csv')
#Num of lines to skip to get to the data
SKIP_TO_DATA_ROW_NUM = 100 
FILE_HEADERS = ['timestamp', 'x', 'y', 'z', 'light', 'button', 'temp']
...
def verify(file_name, subject):
    ...
    gz_match = GZ_FILE_REGEX.match(file_name)
    gz_match_2 = GZ_FILE_REGEX_2.match(file_name)
    if match and match.group('subject') == subject:
        return match
    elif gz_match and gz_match.group('subject') == subject:
        return gz_match
    elif gz_match_2 and gz_match_2.group('subject') == subject:
        return gz_match_2
    elif match_2 and match_2.group('subject') == subject:
        return match_2
    else:
        return None
...
def process_datetime(datetime_string, timezone):
    return datetime.strptime(datetime_string, '%Y-%m-%d %H:%M:%S:%f').replace(tzinfo=tz.gettz(FILE_TIMEZONE)).astimezone(tz.gettz(timezone))

# Parse the file as a dataframe object
def parse(subject, file_path, date_from, timezone, matched_file, write_path):
    if file_path.endswith('csv'):
        return process_data(file_path, date_from, timezone)
...
def process_data(file_object, date_from, timezone):
    ...
    df['date_to'] = df.apply(lambda row: process_datetime(row['timestamp'], timezone), axis=1)
    ...
    for key, alias in VARIABLES.iteritems():
        df_key = 'delta_' + str(key) + '_sq'
        df[df_key] = (df[key].diff())**2

    return get_rms(df.fillna(0))

# Get the RMS value
def get_rms(dataframe_object):
    dataframe_object['activity'] = (dataframe_object['delta_x_sq'] + dataframe_object['delta_y_sq'] + dataframe_object['delta_z_sq'])**0.5
