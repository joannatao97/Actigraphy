...
EXPORT_FILE_PREFIX_DAILY = 'DpreportTextslogDaily'
EXPORT_FILE_PREFIX_HOURLY = 'DpreportTextslogHourly'

VARIABLES = {
    'message length' : 'Totallength'
}
VALUES_TO_PLOT = ['Totallength']
VALUES_RANGE = {
    'Totallength' : {
        'min' : 0,
        'max' : 500
...
def parse(file_path, date_from, timezone, passphrase):
    for row in parser.read_csv(file_path):
        if row['sent vs received'] in ['sent MMS', 'received MMS']:
            continue
