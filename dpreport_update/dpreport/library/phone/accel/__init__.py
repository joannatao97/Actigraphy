...
EXPORT_FILE_PREFIX_HOURLY = 'DpreportAccelHourly'
EXPORT_FILE_PREFIX_DAILY = 'DpreportAccelDaily'
VALUES_TO_PLOT = ['rms']
VALUES_RANGE = {
    'rms' : {
        'min' : 0,
...
# Get the diff for each row for each key; current_row - previous row
# Replace NaNs with 0
...
def process(data_array):
    ...
    for key, alias in VARIABLES.iteritems():
        ...
        dataframe_object[df_key] = (dataframe_object[key].diff())**2

    dataframe_object = dataframe_object.fillna(0)

    return get_rms(dataframe_object)
...
def get_rms(dataframe_object):
    dataframe_object['rms'] = (dataframe_object['delta_x_sq'] + dataframe_object['delta_y_sq'] + dataframe_object['delta_z_sq'])**0.5
...
def process_timeofday(datetime_object):
    return parser.get_timeofday(datetime_object)
