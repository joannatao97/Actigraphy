function [s] = extractRawActData2(dir01, patientstoanalyse, dir2, loadfile)
%
...
%     for patientChoice = 1:1 %length(patients)
    for patientChoice = patientstoanalyse
        ...
        disp('RC extraction...')
        RCmax = NaN([monthsmax, 31, 24, 60]);
        RCvar = RCmax; RCmean = RCmax;
        for j = 1:length(RCu{1})
            for k = 1:length(RCu{2})
                for l = 1:length(RCu{3})
                    for m = 1:length(RCu{4})
                        try
                        a = intersect(find(RCdate(1,:)==RCu{1}(j)),find(RCdate(2,:)==RCu{2}(k)));
                        b = intersect(find(RCdate(3,:)==RCu{3}(l)),find(RCdate(4,:)==RCu{4}(m)));
                        c = intersect(a,b);
                        
                        RCmax(j,RCu{2}(k),RCu{3}(l)+1,RCu{4}(m)+1) = nanmax(RCraw(c));
                        RCmean(j,RCu{2}(k),RCu{3}(l)+1,RCu{4}(m)+1) = nanmean(RCraw(c));
                        RCvar(j,RCu{2}(k),RCu{3}(l)+1,RCu{4}(m)+1) = nanvar(RCraw(c));
                        
                        catch
                            RCmax(j,RCu{2}(k),RCu{3}(l)+1,RCu{4}(m)+1) = NaN;
                            RCmean(j,RCu{2}(k),RCu{3}(l)+1,RCu{4}(m)+1) = NaN;
                            RCvar(j,RCu{2}(k),RCu{3}(l)+1,RCu{4}(m)+1) = NaN;
                        end
                    end
                end
            end
        ...
        if isempty('dir2') == 0
            save([dir2 'extractedActdata.mat'],'s');
        end
