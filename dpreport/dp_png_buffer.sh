...
hsa=`identify *gps_disthome_*.png | awk '{print $3}' | awk -F x '{print $2}'`

for datatype in  surveyAnswers1 surveyAnswers2 gps_disthome accel voice callLog textsLog ACT LIGHT scales qc func
do
	...
	then		
		...
		if [ ${datatype} = "scales" ] || [ ${datatype} = "qc" ]
		then
		...
		if [ `ls -1 | grep -c surveyAnswers` -ge 1 ]
		then
			convert ${i} -background white -fill white -gravity west barx.png -geometry +${posbx}+0 -composite ${i}
		else
			convert ${i} -background white -fill white -gravity northeast -splice ${wbar}x0 ${i}
			convert ${i} -background white -fill white -gravity east barx.png -geometry +0+0 -composite ${i}
		fi
	else
